...
Base2 = "/home/charis/Desktop/Projects/Gesture-Recognition-using-video-/21/05/2026/bilstm"
# GSL original 
...
# Output
OUTPUT_MODEL = os.path.join(Base2, "gsl_Bilstm_merged.pt")
...
class GSLModel(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(
            ...
            batch_first=True,
            bidirectional=True  # This is the parameter that needs to be changed in order to train it as a bidirectional lstm.
        )
    ...
    def forward(self, x):
        ...
        out = torch.cat([h_n[0], h_n[1]], dim=1)  # (B, 256)
...
def plot_classification_report(y_true, y_pred, out_path):
    ...
    table.scale(1.2, 1.5)
    plt.title('Classification Report — BiLSTM', fontsize=14, pad=20)
    plt.tight_layout()
...
def main():
    ...
    report_path = os.path.join(out_dir, "classification_report_BiLSTM.txt")
    ...
    #              os.path.join(out_dir, "training_curves_BiLSTM.png"))
    # plot_confusion(labels, preds,
    #                os.path.join(out_dir, "confusion_matrix_BiLSTM.png"))

    # 7. Plots
    out_dir = Base2
    os.makedirs(out_dir, exist_ok=True)  # ← πρώτα δημιούργησε τον φάκελο
    plot_history(history,
                 os.path.join(out_dir, "training_curves_BiLSTM.png"))
    plot_confusion(labels, preds,
                   os.path.join(out_dir, "confusion_matrix_BiLSTM.png"))
    ...
    os.path.join(out_dir, "classification_report_BiLSTM.png"))
    # 8. Save
    # os.makedirs(out_dir, exist_ok=True)
    torch.save({
